...
//参数为Moudle, for_syclrtc， 而后调用参数为 moudle, outputkind 的Compile函数
...
std::string CodeGenSYCL_Dev::Compile(const ir::Module &module,
  ...
  if (output_kind == OutputKind::CHeader) {
    GenerateHeaderFile(module);//未在这里定义
    ...
    PrintBuiltinCodes();//未实现

    for (auto &func : module.functions()) {
      Compile(func);//对于内部的每个函数，调用参数为 ir::LoweredFunc &func 的Compile函数
    }
  } else {
...
void CodeGenSYCL_Dev::Compile(const ir::LoweredFunc &func) {
  IrPrinter::Visit(Expr(func));//调用visit函数
}
...
std::vector<Expr> CodeGenSYCL_Dev::GenerateBufferAliasExprs(
  ...
  //使用 CollectIRNodes 函数在 op->body 中收集满足特定条件的IR节点（中间表示节点）。条件是节点必须是张量，并且其缓冲区已定义且在 temp_buffer_set 中。
...
//float*  var = (float* )(*(void **)(void_args[0])); 声明变量
